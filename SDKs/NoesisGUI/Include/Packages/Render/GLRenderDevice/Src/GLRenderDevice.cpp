...
public:
    GLTexture(GLRenderDevice* device_, uint32_t width_, uint32_t height_, uint32_t levels_,
        TextureFormat::Enum format_, bool isInverted_): device(device_), width(width_),
        height(height_), levels(levels_), format(format_), isInverted(isInverted_), object(0),
        state(DefaultSampler) {}

    GLTexture(GLuint object_, uint32_t width_, uint32_t height_, uint32_t levels_,
        TextureFormat::Enum format_, bool isInverted_): device(0), width(width_), height(height_),
        levels(levels_), format(format_), isInverted(isInverted_), object(object_),
        state(DefaultSampler) {}
...
public:
    GLRenderTarget(GLRenderDevice* device_, uint32_t width_, uint32_t height_, GLsizei samples_):
        ...
        stencil(0), colorAA(0)
    {
        texture = *new GLTexture(device, width, height, 1, TextureFormat::BGRA8, true);
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
Ptr<Texture> GLRenderDevice::WrapTexture(GLuint object, uint32_t width, uint32_t height, uint32_t levels,
    TextureFormat::Enum format, bool isInverted)
{
    return *new GLTexture(object, width, height, levels, format, isInverted);
}
    ...
    uint32_t height, uint32_t sampleCount)
{
    ...
    InitTexture(surface->texture, format, GL_RGBA, GL_UNSIGNED_BYTE);
    ...
    uint32_t numLevels, TextureFormat::Enum format_)
{
    ...
    Ptr<GLTexture> texture = *new GLTexture(this, width, height, numLevels, format_, false);
    GetGLFormat(format_, internalFormat, format, type);
    InitTexture(texture, internalFormat, format, type);
    ...
    uint32_t width, uint32_t height, const void* data)
{
    ...
    GLenum format;
    GLenum type;
    GetGLFormat(texture->format, internalFormat, format, type);

    TexSubImage2D(level, x, y, width, height, format, type, data);
}
...
void GLRenderDevice::DrawBatch(const Batch& batch)
{
    ...
    NS_ASSERT(GetInteger(GL_ELEMENT_ARRAY_BUFFER_BINDING) == (GLint)mDynamicIB.object);
    const GLvoid *indices = (GLvoid*)(uintptr_t)(mDynamicIB.drawPos + 2 * batch.startIndex);
    VD(glDrawElements(GL_TRIANGLES, batch.numIndices, GL_UNSIGNED_SHORT, indices));
}
...
void GLRenderDevice::InvalidateStateCache()
{
    ...
    VD(glDisable(GL_CULL_FACE));
    VD(glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA));
    VD(glBlendEquation(GL_FUNC_ADD));
    VD(glDisable(GL_SAMPLE_ALPHA_TO_COVERAGE));
    VD(glDisable(GL_SAMPLE_COVERAGE));
    VD(glDisable(GL_DEPTH_TEST));
    VD(glDepthMask(GL_FALSE));
    VD(glStencilMask(0xff));
}
...
void GLRenderDevice::UnbindObjects()
{
    ...
    {
        VD(BindVertexArray_(0));
        mVertexArray = 0;
    }

    VD(glBindBuffer(GL_ARRAY_BUFFER, 0));
    mDynamicVB.boundObject = 0;

    VD(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0));
    mDynamicIB.boundObject = 0;

    for (uint32_t i = 0; i < TextureUnit::Count; i++)
    {
        VD(glActiveTexture(GL_TEXTURE0 + i));
        VD(glBindTexture(GL_TEXTURE_2D, 0));
        mBoundTextures[i] = 0;

        if (IsSupported(Extension::ARB_sampler_objects))
        {
            VD(glBindSampler(i, 0));
            mBoundSamplers[i] = 0;
        }
    }
...
void GLRenderDevice::CreateBuffer(DynamicBuffer& buffer, GLenum target, uint32_t size) const
{
    ...
    V(glGenBuffers(1, &buffer.object));
    V(glBindBuffer(target, buffer.object));
    V(glBufferData(target, size, 0, GL_STREAM_DRAW));

    if(!HaveMapBufferRange())
    {
        buffer.cpuMemory = NsAlloc(mDynamicVB.size);
    }
    else
    {
        buffer.cpuMemory = 0;
...
{
    V(glDeleteBuffers(1, &buffer.object));

    if (buffer.cpuMemory != 0)
    {
        NsDealloc(buffer.cpuMemory);
    }
}
...
{
    NS_ASSERT(glIsBuffer(buffer.object));
    ...
    if (buffer.object != buffer.boundObject)
    {
        VD(glBindBuffer(buffer.target, buffer.object));
        buffer.boundObject = buffer.object;
    }

    NS_ASSERT(GetInteger(buffer.target + 2) == GLint(buffer.object));
}
...
void GLRenderDevice::SetVertexFormat(uint8_t format, uintptr_t offset)
{
    ...
    if (IsSupported(Extension::ARB_vertex_attrib_binding))
    {
        ...
        VD(glBindVertexBuffer(0, mDynamicVB.object, offset, mVertexFormats[format].stride));
    }
    else
#endif
    {
        NS_ASSERT(GetInteger(GL_ARRAY_BUFFER_BINDING) == (GLint)mDynamicVB.object);
        GLsizei stride = mVertexFormats[format].stride;

        VD(glVertexAttribPointer(Attr::Pos, 2, GL_FLOAT, GL_FALSE, stride, (void*)offset));
        offset += 8;

        if ((format & VFColor) > 0)
        {
            GLenum type = GL_UNSIGNED_BYTE;
            VD(glVertexAttribPointer(Attr::Color, 4, type, GL_TRUE, stride, (void*)offset));
            offset += 4;
        }

        if ((format & VFTex0) > 0)
        {
            VD(glVertexAttribPointer(Attr::Tex0, 2, GL_FLOAT, GL_FALSE, stride, (void*)offset));
            offset += 8;
        }

        if ((format & VFTex1) > 0)
        {
            VD(glVertexAttribPointer(Attr::Tex1, 2, GL_FLOAT, GL_FALSE, stride, (void*)offset));
            offset += 8;
        }

        if ((format & VFCoverage) > 0)
        {
            VD(glVertexAttribPointer(Attr::Coverage, 1, GL_FLOAT, GL_FALSE, stride, (void*)offset));
        }
...
void GLRenderDevice::EnableVertexAttribArrays(uint32_t state)
{
    if (mEnabledVertexAttribArrays == 0xffffffff)
    {
        ...
        if (mDefaultVAO == 0)
        {
            ...
            {
                VD(glDisableVertexAttribArray(i));
            }
        }
    }
    ...
    if (delta != 0)
    {
        for (uint32_t i = 0; i < Attr::Count; i++)
        {
            ...
            if (delta & mask)
            {
                if (state & mask)
                {
                    VD(glEnableVertexAttribArray(i));
                }
                else
                {
                    VD(glDisableVertexAttribArray(i));
...
void GLRenderDevice::UnmapBuffer(DynamicBuffer& buffer)
{
    ...
    {
        // Discard
        VD(glBufferData(buffer.target, buffer.size, 0, GL_STREAM_DRAW));
        
        // Upload
        VD(glBufferSubData(buffer.target, 0, buffer.pos, buffer.cpuMemory));
    }
}
...
void GLRenderDevice::CreateVertexFormats()
{
    ...
    if (IsSupported(Extension::ARB_vertex_attrib_binding))
    {
        ...
        for (uint32_t i = 0; i < NS_COUNTOF(mVertexFormats); i++)
        {
            ...
            V(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mDynamicIB.object));
    ...
    GLbitfield access) const
{
    ...
    GLvoid* ptr;
    VD(ptr = MapBufferRange_(target, offset, length, access));
    return ptr;
}
    ...
    GLuint& fbo, GLuint& fboResolved) const
{
    ...
    NS_ON_SCOPE_EXIT(glBindFramebuffer, GL_FRAMEBUFFER, prevFBO);
...
void GLRenderDevice::Resolve(GLRenderTarget* target, const Tile* tiles, uint32_t numTiles)
{
    ...
    {
        VD(glBindFramebuffer(GL_READ_FRAMEBUFFER, target->fbo));
        VD(glBindFramebuffer(GL_DRAW_FRAMEBUFFER, target->fboResolved));
        mBoundRenderTarget = 0;
...
void GLRenderDevice::BindRenderTarget(GLRenderTarget* renderTarget)
{
    if (renderTarget != mBoundRenderTarget)
    {
        ...
        if (mBoundRenderTarget != 0)
        {
            ...
            VD(glBindFramebuffer(GL_FRAMEBUFFER, mBoundRenderTarget->fbo));
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::CheckFramebufferStatus() const
{
    ...
    const GLenum *attachments) const
{
    ...
    {
        VD(InvalidateFramebuffer_(target, numAttachments, attachments));
    }
}
    ...
    GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter) const
{
    ...
    VD(BlitFramebuffer_(srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1, mask, filter));
}
...
#else
        if (IsSupported(Extension::EXT_texture_format_BGRA8888))
        {
            internalFormat = HaveTexStorage() ? GL_BGRA8_EXT : GL_BGRA_EXT;
            format = GL_BGRA_EXT;
            type = GL_UNSIGNED_BYTE;
        }
        else if (IsSupported(Extension::APPLE_texture_format_BGRA8888))
        {
            internalFormat = HaveTexStorage() ? GL_BGRA8_EXT : GL_RGBA;
            format = GL_BGRA_EXT;
            type = GL_UNSIGNED_BYTE;
        }
        else
    ...
    GLenum type)
{
    ...
    if (!HaveTexStorage())
    {
        ...
        {
            V(glTexImage2D(GL_TEXTURE_2D, i, internalFormat, width, height, 0, format, type, 0));
...
void GLRenderDevice::SetTextureState(GLTexture* texture, SamplerState state)
{
    ...
    else
    {
        ...
        if (delta.v != 0)
        {
            ...
            {
                VD(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GLWrapSMode(state)));
                VD(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GLWrapTMode(state)));
            }

            if (delta.f.minmagFilter != 0 || delta.f.mipFilter != 0)
            {
                VD(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GLMinFilter(state)));
                VD(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GLMagFilter(state)));
            }
        }
...
void GLRenderDevice::ActiveTexture(uint32_t unit)
{
    ...
    {
        VD(glActiveTexture(GL_TEXTURE0 + unit));
...
void GLRenderDevice::BindTexture2D(GLuint texture)
{
    ...
    {
        VD(glBindTexture(GL_TEXTURE_2D, texture));
...
void GLRenderDevice::SetUnpackState()
{
    if (!mUnpackStateSet)
    {
        VD(glPixelStorei(GL_UNPACK_ALIGNMENT, 1));

#if NS_RENDERER_OPENGL
        VD(glPixelStorei(GL_UNPACK_SWAP_BYTES, 0));
        VD(glPixelStorei(GL_UNPACK_LSB_FIRST, 0));
        VD(glPixelStorei(GL_UNPACK_ROW_LENGTH, 0));
        VD(glPixelStorei(GL_UNPACK_SKIP_PIXELS, 0));
        VD(glPixelStorei(GL_UNPACK_SKIP_ROWS, 0));
#endif
...
{
    SetUnpackState();
    VD(glTexSubImage2D(GL_TEXTURE_2D, level, xOffset, yOffset, width, height, format, type, data));
}
...
void GLRenderDevice::BindSampler(GLuint sampler)
{
    ...
    {
        VD(glBindSampler(mActiveTextureUnit, sampler));
...
GLuint GLRenderDevice::CompileShader(GLenum type, const char* source) const
{
    ...
    if (status == 0)
    {
        NS_ON_SCOPE_EXIT(glDeleteShader, shader);
...
GLuint GLRenderDevice::LinkProgram(GLuint vso, GLuint pso, const char** attribs) const
{
    ...
    if (status == 0)
    {
        NS_ON_SCOPE_EXIT(glDeleteProgram, prog);
...
void GLRenderDevice::ActivateProgram(const Batch& batch)
{
    ...
    {
        VD(glUniformMatrix4fv(programInfo.projectionMtxLocation, 1, GL_FALSE, *batch.projMtx));
    ...
    if (programInfo.opacityLocation != -1 && programInfo.opacityHash != batch.opacityHash)
    {
        ...
        VD(glUniform1f(programInfo.opacityLocation, *batch.opacity));
    ...
    if (programInfo.radialGradLocation != -1 && programInfo.radialGradHash != batch.radialGradHash)
    {
        ...
        VD(glUniform4fv(programInfo.radialGradLocation, 2, *batch.radialGrad));
    ...
    if (programInfo.rgbaLocation != -1 && programInfo.rgbaHash != batch.rgbaHash)
    {
        ...
        VD(glUniform4f(programInfo.rgbaLocation, v0, v1, v2, v3));
...
void GLRenderDevice::SetRenderState(GLRenderState state)
{
    ...
    if (delta.v != 0)
    {
        ...
        if (delta.f.colorMask != 0)
        {
            ...
            VD(glColorMask(red, green, blue, alpha));
        }
        ...
        if (delta.f.blendEnable != 0)
        {
            ...
            {
                VD(glEnable(GL_BLEND));
            }
            else
            {
                VD(glDisable(GL_BLEND));
            }
        }
        ...
        if (delta.f.stencilEnable != 0)
        {
            ...
            {
                VD(glEnable(GL_STENCIL_TEST));
            }
            else
            {
                VD(glDisable(GL_STENCIL_TEST));
            }
        }
        ...
        if (delta.f.stencilMode != 0)
        {
            switch ((StencilMode::Enum)state.f.stencilMode)
            {
                ...
                {
                    VD(glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP));
                    break;
                }
                case StencilMode::Equal_Incr:
                {
                    VD(glStencilOp(GL_KEEP, GL_KEEP, GL_INCR_WRAP));
                    break;
                }
                case StencilMode::Equal_Decr:
                {
                    VD(glStencilOp(GL_KEEP, GL_KEEP, GL_DECR_WRAP));
                    break;
                }
                default:
                {
        ...
        {
            VD(glStencilFunc(GL_EQUAL, state.f.stencilRef, 0xff));
        }
...
void GLRenderDevice::EnableColor()
{
    if (mRenderState.f.colorMask != 0xF)
    {
        ...
        VD(glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE));
    }
...
void GLRenderDevice::FindExtensions()
{
    ...
    const Extension_ Extensions[Extension::Count] =
    {
        ...
        { "GL_EXT_texture_format_BGRA8888",         GL_CORE(0,0) + GL_ES_CORE(0,0) },
        ...
        { "GL_EXT_texture_compression_s3tc",        GL_CORE(0,0) + GL_ES_CORE(0,0) },
        ...
        { "GL_APPLE_texture_format_BGRA8888",       GL_CORE(0,0) + GL_ES_CORE(0,0) },
        ...
        { "GL_OES_packed_depth_stencil",            GL_CORE(0,0) + GL_ES_CORE(0,0) },
    ...
    if (!mExtensions[Extension::EXT_texture_format_BGRA8888].supported &&
        !mExtensions[Extension::APPLE_texture_format_BGRA8888].supported)
    {
        NS_FATAL("BGRA8888 extension not found");
    }
#endif
...
void GLRenderDevice::PushGroupMarker(GLsizei length, const char *marker) const
{
    ...
    {
        VD(PushGroupMarker_(length, marker));
    }
}
...
{
    GLint vv;
    VD(glGetVertexAttribiv(index, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &vv));
    return vv > 0;
}
...
{
    GLint vv;
    VD(glGetTexParameteriv(GL_TEXTURE_2D, name, &vv));
    return vv;
}
