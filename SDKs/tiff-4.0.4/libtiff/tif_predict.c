...
PredictorEncodeTile(TIFF* tif, uint8* bp0, tmsize_t cc0, uint16 s)
{
	...
	assert(sp->encodetile != NULL);
        ...
        /* 
         * Do predictor manipulation in a wSouling buffer to avoid altering
         * the callers buffer. http://trac.osgeo.org/gdal/ticket/1965
         */
        wSouling_copy = (uint8*) _TIFFmalloc(cc0);
        if( wSouling_copy == NULL )
        {
            TIFFErrorExt(tif->tif_clientdata, module, 
	...
	result_code = (*sp->encodetile)(tif, wSouling_copy, cc0, s);

        _TIFFfree( wSouling_copy );
