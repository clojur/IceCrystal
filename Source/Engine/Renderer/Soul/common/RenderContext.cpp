...
bool Soul::RenderContext::initRenderContext(HWINDOW hWnd)
{
	...
	if (glewInit() != GLEW_OK)
	{
		Soul::Logger::ERROR_LOGGER->log("SoulInit","glewInit Failed!");
		Soul::Logger::ERROR_LOGGER->flush();
		return false;
	}
	...
	{
		Soul::Logger::ERROR_LOGGER->log("SoulInit", "setup pixel format Failed!");
		Soul::Logger::ERROR_LOGGER->flush();
		return false;
	}
	GLint attribs[] = {
	...
	if (_hGlrc == NULL)
	{
		Soul::Logger::ERROR_LOGGER->log("SoulInit", "create opengl3.3 failed!\nattempt create opengl3.2");
		Soul::Logger::ERROR_LOGGER->flush();
		attribs[3] = 2;
		_hGlrc = wglCreateContextAttribsARB(_hDisplay, 0, attribs);
		if (_hGlrc == NULL)
		{
			Soul::Logger::ERROR_LOGGER->log("SoulInit", "create opengl3.2 failed!");
			Soul::Logger::ERROR_LOGGER->flush();
			return false;
		}
	}
