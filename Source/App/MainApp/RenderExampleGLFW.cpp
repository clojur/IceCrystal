...
class RenderExampleGLFW : public GlfwWindow
{
public:
    ...
    RenderExampleGLFW(const string &dir) :
...
static_ptr<Window> RenderExampleGLFW::app;

int renderExampleGLFW(int argc, char* argv[])
{
    assert(argc > 2);
    atexit(Object::exit);
    RenderExampleGLFW::app = new RenderExampleGLFW(string(argv[2]));
    RenderExampleGLFW::app->start();
    return 0;
}

MainFunction renderGLFWMain("renderglfw", renderExampleGLFW);
